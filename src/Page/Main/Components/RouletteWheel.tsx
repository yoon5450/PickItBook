...
function RouletteWheel({
  ...
  const getPinOffsetDeg = (
    wheelEl: HTMLDivElement,
    pinEl: HTMLImageElement
  ) => {
  ...
  const runRoulette = () => {
    ...
    let prevIdx =
      Math.round(normalizeAngle(pinAngle - prevAngle) / sliceDeg) % total;
    ...
    const kickPin = (speed: number) => {
      const kick = gsap.utils.clamp(
        3,
        18,
        gsap.utils.mapRange(0, 25, 4, 18, Math.abs(speed))
      );
      gsap.fromTo(
        pin,
        { rotation: -kick },
    ...
    if (repeat === -1) {
      ...
      gsap.to(wheel, {
        ...
        onUpdate: () => {
          ...
          const idx =
            Math.round(normalizeAngle(pinAngle - curAngle) / sliceDeg) % total;
          if (idx !== prevIdx) {
            kickPin(speed);
            prevIdx = idx;
          }
  ...
  // === 버튼 클릭 여부에 따라 룰렛 실행 ===
  useEffect(() => {
    console.log("isStart 변경됨:", isStart);
    if (!isStart) return;
    console.log("runRoulette 실행");
    runRoulette();
  }, [isStart]);
