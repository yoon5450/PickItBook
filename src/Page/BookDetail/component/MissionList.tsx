...
function MissionList({ items }: Props) {
  return (
    <ul className="flex flex-col w-full gap-2 pt-4">
      ...
      {items.map(
        ({ missionType, missionTitle, score, userArchiveRate, isComplete }) => (
          <li className="relative flex justify-between items-center py-4 px-8 bg-white rounded-md">
            <div className="flex flex-col gap-1">
              <p className="text-gray-300">{missionType}</p>
              <p className="text-[20px]">{missionTitle}</p>
            </div>
