...
interface SummaryModalProps {
  ...
  // Optional: pass missions from parent. If omitted, a small demo list renders.
  relatedMissions?: { id: string | number; title: string; reward?: string }[];
  bookTitle?: string;
}

const DEMO_MISSIONS = [
  { id: 1, title: "3줄 요약 제출", reward: "+50 XP" },
  { id: 2, title: "인상 깊은 구절 남기기", reward: "+30 XP" },
];
...
export default function SummaryModal({
  ...
  const _missions = relatedMissions && relatedMissions.length > 0 ? relatedMissions : DEMO_MISSIONS;
  ...
  const handleSave = async () => {
    ...
    try {
      onSave?.(summaryLines);
      // TODO: Replace with real persistence logic (RPC / mutation)
      // console.log({ summary: summaryLines });
      onClose();
    } finally {
      setSaving(false);
    }
      ...
      <div className="mx-4 w-full max-w-[880px] overflow-hidden rounded-2xl bg-white shadow-2xl">
        ...
        <div className="flex items-center gap-3 border-b border-gray-100 px-6 py-4">
          ...
          <div className="min-w-0 flex-1">
            <h2 id="summary-modal-title" className="truncate text-xl font-bold text-gray-900">
              3줄 요약
            </h2>
            <p className="truncate text-sm text-gray-500">
              {bookTitle ? `${bookTitle}에 대해 3줄로 정리해 보세요.` : "핵심만 간결하게 기록하면 나중에 다시 볼 때 좋아요."}
            </p>
          </div>
          <button
            onClick={onClose}
        ...
        <div className="flex flex-col gap-6 px-6 py-6 md:grid-cols-5">
          ...
          <div className="md:col-span-3">
            <div className="mb-3 flex items-center justify-between">
              <span className="text-sm text-gray-500">Why 3 lines? 한 문장당 하나의 핵심!</span>
              <span className="text-xs text-gray-400">Enter로 줄바꿈 가능 / 최대 140자</span>
            </div>
                ...
                <div key={i} className="flex items-start gap-3">
                  ...
                  <div className="relative w-full">
                    <textarea
                      ...
                      onChange={(e) => handleSummaryChange(i, e.target.value.slice(0, 140))}
                      rows={2}
                      className={tw(
                      ...
                      aria-label={`요약 ${i + 1}`}
                    />
                    <span className="pointer-events-none absolute bottom-1 right-2 text-xs text-gray-400">
            ...
            <div className="rounded-xl border border-amber-200 bg-amber-50/60 p-4">
              <div className="mb-2 flex items-center gap-2">
                ...
                <span className="text-sm text-amber-800">
                  이 책과 연관된 미션이 {_missions.length}개 있어요
                </span>
              </div>
              <ul className="space-y-2">
                {_missions.map((m) => (
                  <li key={m.id} className="flex items-center justify-between rounded-lg bg-white px-3 py-2 ring-1 ring-amber-200">
                    <span className="truncate text-sm font-medium text-amber-900">{m.title}</span>
                    {m.reward && <span className="ml-3 shrink-0 text-xs text-amber-600">{m.reward}</span>}
                  </li>
                ))}
              </ul>
